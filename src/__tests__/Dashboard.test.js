...
describe("Given I am connected as an Admin", () => {
  ...
  describe("When I am on Dashboard page and I click on arrow", () => {
    ...
    describe("When I am on Dashboard page and I already click on 1st an arrow to display the bills, if I click again on the 1st arrow ", () => {
      it("should remove and hide the bills", async () => {
        const onNavigate = (pathname) => {
          document.body.innerHTML = ROUTES({ pathname });
        };
        const dashboard = new Dashboard({
        ...
        expect(handleShowTickets1).toHaveBeenCalled();
        await waitFor(() => screen.getByTestId(`open-bill47qAXb6fIm2zOKkLzMro`));
        await waitFor(() => screen.getByTestId(`bill-cards-container`));
        const billCardContainers = screen.getByTestId(`bill-cards-container`);
        expect(screen.getByTestId(`open-bill47qAXb6fIm2zOKkLzMro`)).toBeTruthy();
        expect(billCardContainers.classList).toContain("displayed");
        expect(billCardContainers.firstChild).toBeTruthy();
        userEvent.click(icon1);
        expect(handleShowTickets1).toHaveBeenCalled();
        expect(billCardContainers.classList).toContain("hidden");
        expect(billCardContainers.firstChild).toBeFalsy();
      });
    });
  });
  ...
  describe("When I am on Dashboard page and I click on a card", () => {
    test("Then, big bill Icon should disappear", () => {
      const onNavigate = (pathname) => {
        document.body.innerHTML = ROUTES({ pathname });
      };
      ...
      const card = screen.getByTestId("open-bill47qAXb6fIm2zOKkLzMro");
